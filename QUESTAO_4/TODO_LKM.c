...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   ...
   error_count = copy_to_user(buffer, &(todo_head.descricao), size_of_message);
   ...
   if (error_count==0){            // if true then have success
      ...
      printk(KERN_INFO "TODO list: Falha ao enviar %d caracteres para o usuario\n", error_count);
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
    ...
    struct todo_list * tmp = (struct todo_list *) kmalloc(sizeof(struct todo_list), GFP_ATOMIC);

    sprintf(tmp->descricao, "%s(%zu letras)", buffer, len);   // appending received string with its length
    size_of_message = strlen(message);                        // store the length of the stored message

    list_add_tail(&(tmp->list), &todo_head); //Adiciona os nodos

    printk(KERN_INFO "TODO list: Recebido %zu caracteres do usuario\n", len);
    return len;
}
